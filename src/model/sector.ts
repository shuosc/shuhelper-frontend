...
export namespace SectorService {
    export function sectorIndexForTime(dateTime: Date) {
        return findIndex(partial(isWithinInterval, extractTime(dateTime)))(SectorRepository.sectors);
    }

    export function restIndexForTime(dateTime: Date) {
        return findIndex(partial(isWithinInterval, extractTime(dateTime)))(SectorRepository.rests);
    }

    export function restForTime(dateTime: Date) {
        return findFirst((x: Interval) => isWithinInterval(extractTime(dateTime), x))(SectorRepository.rests);
    }
}
